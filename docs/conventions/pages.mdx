...
7. **Copy from `constants/`** — titles, empty/error strings live in `src/constants/pages.ts` (see [constants](/?path=/docs/conventions-constants--docs)). Date helpers live in `src/lib/date.ts` (see [lib](/?path=/docs/conventions-lib--docs)).
8. **Extract feature UI** — rich list cards / sections belong in `components/features/`, not inline in `*-page.tsx` (see [folder](/?path=/docs/conventions-folder--docs)).
