# Components

Catalog of `@erp/miniapp-ui` primitives. Each page follows: Purpose, Import, Props, Variants, Sizes, Accessibility, Do/Don't, Example, API, Source.

| Component | Id |
| --- | --- |
| [Accordion](./accordion.md) | `accordion` |
| [Alert Dialog](./alert-dialog.md) | `alert-dialog` |
| [Alert](./alert.md) | `alert` |
| [Aspect Ratio](./aspect-ratio.md) | `aspect-ratio` |
| [Avatar](./avatar.md) | `avatar` |
| [Badge](./badge.md) | `badge` |
| [Bar Chart](./bar-chart.md) | `bar-chart` |
| [Breadcrumb](./breadcrumb.md) | `breadcrumb` |
| [Button](./button.md) | `button` |
| [Calendar](./calendar.md) | `calendar` |
| [Card](./card.md) | `card` |
| [Checkbox](./checkbox.md) | `checkbox` |
| [Collapsible](./collapsible.md) | `collapsible` |
| [Dialog](./dialog.md) | `dialog` |
| [Donut Chart](./donut-chart.md) | `donut-chart` |
| [Drawer](./drawer.md) | `drawer` |
| [Dropdown Menu](./dropdown-menu.md) | `dropdown-menu` |
| [Empty](./empty.md) | `empty` |
| [Field](./field.md) | `field` |
| [Hover Card](./hover-card.md) | `hover-card` |
| [Input](./input.md) | `input` |
| [Label](./label.md) | `label` |
| [Line Chart](./line-chart.md) | `line-chart` |
| [Pagination](./pagination.md) | `pagination` |
| [Popover](./popover.md) | `popover` |
| [Progress](./progress.md) | `progress` |
| [Radio Group](./radio-group.md) | `radio-group` |
| [Scroll Area](./scroll-area.md) | `scroll-area` |
| [Select](./select.md) | `select` |
| [Separator](./separator.md) | `separator` |
| [Sheet](./sheet.md) | `sheet` |
| [Sidebar](./sidebar.md) | `sidebar` |
| [Skeleton](./skeleton.md) | `skeleton` |
| [Slider](./slider.md) | `slider` |
| [Spinner](./spinner.md) | `spinner` |
| [Switch](./switch.md) | `switch` |
| [Table](./table.md) | `table` |
| [Tabs](./tabs.md) | `tabs` |
| [Textarea](./textarea.md) | `textarea` |
| [Toast (Sonner)](./sonner.md) | `sonner` |
| [Toggle](./toggle.md) | `toggle` |
| [Toggle Group](./toggle-group.md) | `toggle-group` |
| [Tooltip](./tooltip.md) | `tooltip` |
